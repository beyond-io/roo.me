...
class TestModels:
    ...
    def test_city_creation(self, city_model):
        assert city_model is not None
        assert city_model.cityName == 'nice_city'

    def test_get_city(self, city_model):
        check_city = City.objects.get(cityName=city_model.cityName)
        assert check_city == city_model

    def test_apartment_creation(self, apartment_model):
        assert apartment_model is not None
        assert apartment_model.address == 'street'
        assert apartment_model.rent == 4500
        assert apartment_model.is_relevant is True

    def test_get_apartment(self, apartment_model):
        check_apartment = Apartment.objects.get(owner=apartment_model.owner)
        assert check_apartment == apartment_model
